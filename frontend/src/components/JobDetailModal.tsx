...
import { X, ExternalLink, MapPin, Building, Calendar, DollarSign, Database, ShieldCheck } from 'lucide-react';
        ...
        {job.tags && job.tags.length > 0 && (
          <div className="flex flex-wrap gap-2 mt-5 pt-5 border-t border-white/10">
            {job.tags.map((tag, idx) => (
              <span
                key={idx}
                className="px-3 py-1 rounded-lg text-xs font-semibold bg-gray-900/90 text-gray-200 border border-white/10"
              >
                #{tag}
              </span>
            ))}
